CLASS zcl_range_to_intervals__time DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      ...
      END OF ts_interval.
    TYPES tt_interval TYPE SORTED TABLE OF ts_interval WITH UNIQUE KEY primary_key ALIAS by_from_time COMPONENTS from.
...
CLASS zcl_range_to_intervals__time IMPLEMENTATION.
  METHOD constructor.
    super->constructor( ).
    gr_time = ir_time.
  ENDMETHOD.
  ...
  METHOD get_intervals.
    ...
    DATA(interval) = VALUE ts_interval( from = '000000' ).

    LOOP AT splittings REFERENCE INTO DATA(splitting).
      " Finish the interval (FROM_TIME already initialized)
      interval-to     = splitting->value_before.
      interval-is_in_range = boolc( interval-from IN gr_time ).
